...
import {
  ...
  ALLOWED_IMAGE_TYPES,
  ALLOWED_VIDEO_TYPES,
} from "@/data/lib/constants";
import { MediaFile } from "@/data/types/postTypes/media";
import { validateFile, getInitials } from "@/data/utils/file-helpers";
  ...
  const handleFileChange = async (
    ...
    for (const file of files) {
      ...
      if (validation.isVideo !== isVideoUpload) {
        toast({
          title: "Error",
          description: `Por favor, use el botón correcto para subir ${
            isVideoUpload ? "videos" : "imágenes"
          }`,
          variant: "destructive",
        });
        continue;
        ...
        <form onSubmit={handleUpload}>
          {nUser && (
            <div className="flex items-center space-x-4">
              <Avatar>
                <AvatarImage
                  src={nUser.avatar_url ?? ""}
                  alt={nUser.display_name ?? "User"}
                />
                <AvatarFallback>
                  {nUser.display_name ? getInitials(nUser.display_name) : "U"}
                </AvatarFallback>
              </Avatar>
              <Textarea
          ...
          {hashtags.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {hashtags.map((tag, index) => (
                <span
                  key={index}
                  className="bg-blue-100 text-blue-800 dark:bg-blue-600 dark:text-blue-200 text-xs font-semibold px-2.5 py-0.5 rounded"
                >
                  #{tag}
                </span>
              ))}
