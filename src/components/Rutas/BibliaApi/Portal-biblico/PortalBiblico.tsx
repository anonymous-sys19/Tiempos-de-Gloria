...
export default function PortalBiblico() {
  ...
  const filteredEvents = verses.filter((event) =>
    quitarTildes(event.tema).toLowerCase().includes(quitarTildes(searchTerm).toLowerCase()) ||
    quitarTildes(event.rol).toLowerCase().includes(quitarTildes(searchTerm).toLowerCase())
  );
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto p-4 bg-gray-100 min-h-screen">
        <h1 className="text-3xl font-bold text-center mb-6">Línea de Tiempo Bíblica</h1>
        <div className="mb-4">
          <Input
            type="text"
          ...
          {filteredEvents.map((event) => (
            <Card key={event.id} className="mb-6">
              <CardHeader className="flex flex-row items-center gap-4">
                <Avatar>
                  <AvatarImage src={`https://api.dicebear.com/6.x/initials/svg?seed=${event.tema}`} />
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="icon" onClick={() => handleShare(event)}>
                    <Share2 className="h-4 w-4" />
                    <span className="sr-only">Compartir</span>
                  </Button>
                  <Button variant="outline" size="icon" onClick={() => handleCopy(event)}>
                    <Copy className="h-4 w-4" />
                    <span className="sr-only">Copiar</span>
                  </Button>
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="outline" onClick={() => setSelectedVerse(event)}>Ver más</Button>
                    </DialogTrigger>
                    <DialogContent className="sm:max-w-[425px]">
                      ...
                      <div className="mt-4">
                        <p className="text-sm text-gray-500 mb-2">{selectedVerse?.pasaje}</p>
