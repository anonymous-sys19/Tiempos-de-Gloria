...
import { ChurchNode, D3Node } from "@/data/types/ChurchData";
import { getNodeColor, getLinkColor } from "@/data/utils/colors";
  ...
  useEffect(() => {
    ...
    function update(source: D3Node) {
      ...
      nodeEnter
        ...
        .style(
          "stroke",
          (d) => d3.color(getNodeColor(d.depth))?.darker().toString() ?? "#000"
        )
      ...
      nodeEnter
        ...
        .style(
          "fill",
          (d) => d3.color(getNodeColor(d.depth))?.darker(2).toString() ?? "#000"
        )
      ...
      // Update links
      const link = g
        .selectAll<SVGPathElement, d3.HierarchyLink<ChurchNode>>("path.link")
      ...
      const linkEnter = link
        ...
        .style(
          "stroke-width",
          (d) => `${Math.max(3 - d.source.depth * 0.5, 1)}px`
        )
        .style("opacity", 0.7);
