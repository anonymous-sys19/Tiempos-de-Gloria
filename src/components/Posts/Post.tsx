...
import { formatDate } from "@/data/utils/utils-format";
import { PostTypes, Comment } from "@/data/types/postTypes/posts";
  ...
  ({
    ...
    const fetchComments = async () => {
      ...
      const likedCommentIds = new Set<string>(
        likedCommentsData.map(({ comment_id }) => comment_id)
      );
      ...
      try {
        if (hasLiked) {
          ...
          if (!error) {
            ...
            toast({
              title: "Like retirado",
              description: "Ya no te gusta esta publicación",
            });
          }
        } else {
          const { error } = await supabase
        ...
        toast({
          title: "Error",
          description: "No se pudo actualizar el estado del like",
        });
      }
    };
      ...
      if (platform === "whatsapp") {
        window.open(
          `https://wa.me/?text=${encodeURIComponent(
            `¡Mira esta publicación! ${shareUrl}`
          )}`,
          "_blank"
        );
    ...
    const handleLikeComment = async (commentId: string) => {
      ...
      try {
        if (isLiked) {
          ...
          toast({
            title: "Like retirado",
            description: "Ya no te gusta este comentario",
          });
        } else {
          const { error } = await supabase.from("comments_liked").insert([
            {
          ...
          toast({
            title: "Like agregado",
            description: "Te gusta este comentario",
          });
        }
      } catch (error) {
        ...
        <Card className="w-full max-w-3xl bg-white shadow-md rounded-lg">
          ...
          <div className="p-4">
            ...
            <div className="flex justify-between">
              <Button
                variant="outline"
                className={`flex-1 flex items-center justify-center border-none ${
              ...
              >
                <ThumbsUp
                  className={`mr-2 h-4 w-4 ${
                    hasLiked ? "fill-current text-blue-500" : ""
                  }`}
                />
            ...
            <DialogContent>
              ...
              <div className="flex flex-col space-y-4">
                ...
                <div className="flex space-x-2">
                  ...
                  >
                    <MessageCircle className="mr-2 h-4 w-4" /> Compartir en
                    WhatsApp
                  </Button>
                </div>
              </div>
