...
export function ChatApp() {
            ...
            if (!documentHasFocus) {
              ...
              if (senderData) {
                showNotification(
                  ...
                  newMessage.content || "Sent an image"
                );
              }
  ...
  const fetchConversations = async () => {
    ...
    try {
      ...
      const sortedConversations = conversationsData.sort((a, b) => {
        ...
        return (b.lastMessage?.created_at || "").localeCompare(
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const unreadMessages =
        data?.filter((msg) => msg.receiver_id === user.id && !msg.is_read) ||
        [];
  ...
  const handleSendMessage = async () => {
    if (!user || !selectedUser || (!newMessage.trim() && !selectedImage))
      return;
          ...
          {conversations.map((conversation) => (
            <button
              ...
              className={`w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-gray-100 ${
                selectedUser?.id === conversation.profile.id
                  ? "bg-gray-200"
                  : ""
              }`}
                ...
                <div className="flex items-center gap-1 justify-between">
                  ...
                  {!conversation.profile.is_online && (
                    <span className="text-xs text-red-800">
                      Last seen{" "}
                      {formatDistanceToNow(
                        new Date(conversation.profile.last_seen),
                        { addSuffix: true }
                      )}
                    </span>
                  )}
                      ...
                      {message.replied_message && (
                        <div className="text-sm bg-[#f0f2f5] p-2 rounded mb-2 border-l-4 border-[#25d366]">
                          <p className="font-medium text-[#25d366]">
                            {message.replied_message.sender_id === user?.id
                              ? "You"
                              : message.replied_message.profiles?.display_name}
                          </p>
